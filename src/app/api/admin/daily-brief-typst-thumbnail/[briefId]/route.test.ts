...
const { renderDailyBriefTypstThumbnailPngMock } = vi.hoisted(() => ({
  renderDailyBriefTypstThumbnailPngMock: vi.fn(),
}));

vi.mock("../../../../../lib/daily-brief-thumbnail", () => ({
  renderDailyBriefTypstThumbnailPng: (...args: unknown[]) =>
    renderDailyBriefTypstThumbnailPngMock(...args),
}));

import { POST as adminLogin } from "../../login/route";
import { GET as dailyBriefTypstThumbnailRoute } from "./route";
...
beforeEach(async () => {
  tempDirectory = await mkdtemp(
    path.join(tmpdir(), "daily-sparks-admin-typst-thumbnail-"),
  );
  ...
  renderDailyBriefTypstThumbnailPngMock.mockReset();
});
...
describe("admin daily brief typst thumbnail route", () => {
  test("rejects unauthenticated requests", async () => {
    const response = await dailyBriefTypstThumbnailRoute(
      new Request(
        "http://localhost:3000/api/admin/daily-brief-typst-thumbnail/brief-1",
      ),
      {
  ...
  test("returns a png thumbnail for an existing daily brief typst prototype", async () => {
    ...
    renderDailyBriefTypstThumbnailPngMock.mockResolvedValue(pngBuffer);

    const response = await dailyBriefTypstThumbnailRoute(
      new Request(
        `http://localhost:3000/api/admin/daily-brief-typst-thumbnail/${brief.id}`,
        {
          headers: {
            cookie,
          },
    ...
    expect(renderDailyBriefTypstThumbnailPngMock).toHaveBeenCalledWith(
