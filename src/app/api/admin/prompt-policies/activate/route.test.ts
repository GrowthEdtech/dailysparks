...
import { POST as activatePromptPolicyRoute } from "./route";
...
beforeEach(async () => {
  tempDirectory = await mkdtemp(
    path.join(tmpdir(), "daily-sparks-prompt-policy-activate-"),
  );
...
describe("activate prompt policy route", () => {
  test("activates a draft prompt policy", async () => {
    ...
    const activateResponse = await activatePromptPolicyRoute(
      new Request(
        "http://localhost:3000/api/admin/prompt-policies/activate",
        {
          method: "POST",
          headers: { cookie, "content-type": "application/json" },
          body: JSON.stringify({
    ...
    expect(activateBody.policy.status).toBe("active");
  });
});
