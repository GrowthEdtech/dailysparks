import { activatePromptPolicy } from "../../../../../lib/prompt-policy-store";
import {
...
export async function POST(request: Request) {
  ...
  if (!id) {
    return badRequest("Please choose a prompt policy to activate.");
  }

  const policy = await activatePromptPolicy(id);

  if (!policy) {
    return Response.json(
      { message: "We could not activate that prompt policy." },
      { status: 404 },
    );
  }
