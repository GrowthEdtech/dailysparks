import { archivePromptPolicy } from "../../../../../lib/prompt-policy-store";
import {
...
export async function POST(request: Request) {
  ...
  if (!id) {
    return badRequest("Please choose a prompt policy to archive.");
  }

  const policy = await archivePromptPolicy(id);

  if (!policy) {
    return Response.json(
      { message: "We could not archive that prompt policy." },
      { status: 404 },
    );
  }
