...
import { POST as archivePromptPolicyRoute } from "./route";
...
beforeEach(async () => {
  tempDirectory = await mkdtemp(
    path.join(tmpdir(), "daily-sparks-prompt-policy-archive-"),
  );
...
describe("archive prompt policy route", () => {
  test("archives a draft prompt policy", async () => {
    const cookie = await signIn();

    await createPromptPolicyRoute(
    ...
    const archiveResponse = await archivePromptPolicyRoute(
      new Request("http://localhost:3000/api/admin/prompt-policies/archive", {
        method: "POST",
        headers: { cookie, "content-type": "application/json" },
        body: JSON.stringify({
    ...
    expect(archiveBody.policy.status).toBe("archived");
  });
});
